...
// 17fc54c872bd8ceabf271a7e9e636c7c583f55af
...
public:
  ColorGradientInitialPDFsSetterSinglePrecisionCUDA(
      ...
      BlockDataID rho_bID_, BlockDataID velocityID_)
      : force_aID(force_aID_), force_bID(force_bID_), pdfs_aID(pdfs_aID_),
        ...
        rho_bID(rho_bID_), velocityID(velocityID_) {}
  ...
  getSweep(const shared_ptr<ColorGradientInitialPDFsSetterSinglePrecisionCUDA>
               &kernel) {
    return [kernel](IBlock *b) { kernel->run(b); };
  }

  static std::function<void(IBlock *, gpuStream_t)> getSweepOnCellInterval(
      const shared_ptr<ColorGradientInitialPDFsSetterSinglePrecisionCUDA>
          &kernel,
